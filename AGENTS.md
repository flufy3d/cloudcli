...
- Publish a new build: `pnpm run deploy` (build → pack → isolated staging install → fixed-runtime cutover → PM2 restart → health check → `pm2 save`); the script auto-increments the patch version and commits it. Deployments are serialized by `~/.cloudcli/deploy.lock`, retain the previous runtime for automatic rollback, and update the global `cloudcli` command to point at the fixed runtime. Release-pipeline deploys pass `--no-bump` to keep the tagged release version instead. Run it in a terminal **outside** any cloudcli-hosted session — the cutover drops the session's own server.
