...
模块内的次级排序契约见 `sessionTimelineStore.ts` 头注释：内容帧先 flush 流式缓冲再落表（路由表的 flush 门）；服务端覆盖剪枝必须先于内容级短路；旧页拉取期间的偏移漂移要先做一次有界最新页校准；流式行时间戳锚定在分段开始且不刷新；历史与实时消息以稳定双序列合并，时间戳只决定两路的大致交错位置，各路内部顺序不可被另一台机器的时钟重排。
